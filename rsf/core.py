...
from .utils import quote, unquote, fromstring, file2str, str2file, errexit
...
def _check_key(key):
    if key not in RSFKEYS_READ + ("ax1", "ax2", "ax3", "ax4", "ax5", "ax6", "ax7", "ax8", "ax9"):
...
class RSF:
    ...
    def read_rsf(self, filename):
        ...
        if not infile or not os.path.exists(infile):
            errexit("Error: Cannot find 'in' file to read")
